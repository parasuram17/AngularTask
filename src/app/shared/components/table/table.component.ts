...
export class TableComponent {
  ...
  innerCheckBoxClick(event: Event) {
    let cbId = (event.target as HTMLInputElement).id
    let empId = cbId.replaceAll('-cb', '');
  ...
  ellipseViewClick(event: Event) {
    let target = event.target as HTMLElement;
    let ellipseView = target.closest(".tt-view") as HTMLAnchorElement;
    let empNo = ellipseView.id
    empNo = empNo.replaceAll('-child', '')
    this.router.navigate(['/auth/employee/add'], { queryParams: { viewMode: true, employeeId: empNo } });
  }

  ellipseEditClick(event: Event) {
    let target = event.target as HTMLElement;
    let ellipseEdit = target.closest(".tt-edit") as HTMLAnchorElement;
    let empNo = ellipseEdit.id
    empNo = empNo.replaceAll('-child2', '')
    this.router.navigate(['/auth/employee/add'], { queryParams: { editMode: true, employeeId: empNo } });
  }

  ellipseDeleteClick(event: Event) {
    let target = event.target as HTMLElement;
    let ellipseDelete = target.closest(".tt-delete") as HTMLAnchorElement;
    let empNo = ellipseDelete.id
