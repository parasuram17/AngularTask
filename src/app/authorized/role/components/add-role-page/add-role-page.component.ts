      ...
      if (url.queryParamMap.has('editMode')) {
        this._roleService.updateData(roleRequest.id, roleRequest).subscribe({
          next: (response) => {
            ...
            localStorage.setItem("Toast", JSON.stringify(["success", "Role has been updated successfully!"]));
            this.router.navigate(['/auth/role'])
          },
          error: (err) => {
            localStorage.setItem("Toast", JSON.stringify(["failure", "Error occured while editing the role!"]));
            this.router.navigate(['/auth/role'])
          }
        })
      }
        ...
        this._roleService.postData(roleRequest).subscribe({
          next: (response) => {
            ...
            localStorage.setItem("Toast", JSON.stringify(["success", "Role has been successfully added!"]));
            this.router.navigate(['/auth/role'])
          },
          error: (err) => {
            localStorage.setItem("Toast", JSON.stringify(["failure", "Error occured while adding the role!"]));
            this.router.navigate(['/auth/role'])
          }
        })
      }
