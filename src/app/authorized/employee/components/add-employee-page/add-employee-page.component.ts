    ...
    if (this.employeeForm.valid) {
      ...
      if (url.queryParamMap.has('editMode') == true) {
        this._employeeService.updateData(employeeRequest.id, employeeRequest).subscribe({
          next: (response) =>{
            localStorage.setItem("Toast", JSON.stringify(["success", "Employee has been successfully updated!"]));
            this.router.navigate(['/auth/employee']);

          },
          error: (err) =>{
            localStorage.setItem("Toast", JSON.stringify(["failure", "Error occured while updating the employee!"]));
            this.router.navigate(['/auth/employee']);
          }
        })
      }
        ...
        this._employeeService.postData(employeeRequest).subscribe({
          next: (response) => {
            localStorage.setItem("Toast", JSON.stringify(["success", "Employee has been successfully added!"]));
            this.router.navigate(['/auth/employee']);
          },
          error: (err) => {
            localStorage.setItem("Toast", JSON.stringify(["failure", "Error occured while adding the employee!"]));
            this.router.navigate(['/auth/employee']);
          }
        })
      }
